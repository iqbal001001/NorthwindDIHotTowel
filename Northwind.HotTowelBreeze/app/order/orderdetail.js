(function () {
    ...
    var controllerId = 'orderdetail';
    angular.module('app').
        controller(controllerId,
        ['$routeParams', '$location', '$scope', '$rootScope', '$window',
            'bootstrap.dialog', 'common', 'config', 'datacontext',
            'helper', 'model',  orderdetail]);

    function orderdetail($routeParams, $location, $scope, $rootScope, $window,
        ...
        function activate() {
            ...
            common.activateController([getRequestedOrder(), getEmployees(), getCustomers()], controllerId)
             .then(onEveryChange);
            //log('Activated Orderdetail View');
            //});
        }
        ...
        function getRequestedOrder() {
            ...
            .then(function (data) {
                ...
                $rootScope.transactions = vm.order.order_Details;
            }, function (error) {
        ...
        function onTotalChange() {
            $scope.$on('orderTotalChange',
                function (event, data) {
                    if (data.data.orderID == vm.order.orderID) {
                        vm.order.total = data.data.total;
                    }
                });
        }
        ...
        function storeWipEntity() {
            if (!vm.order) return;
            var description = vm.order.orderID || '[New Order]' + vm.order.orderID
